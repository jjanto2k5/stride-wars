...
import { X, Trophy, Map, Activity, RefreshCw } from 'lucide-react';
...
export default function LeaderboardPanel({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => fetchLeaderboard(controller.signal, true), 30000);
  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed top-0 right-0 h-[100dvh] w-[85vw] max-w-sm bg-gray-900 border-l border-gray-800 z-[2001] shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-800 bg-gray-900 z-10">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-white p-2 transition-colors" aria-label="Close leaderboard">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24">
          ...
          {!loading && error && (
            <div className="flex flex-col items-center justify-center text-center py-12 px-4">
              ...
              <button onClick={handleRetry} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-3 rounded-xl font-semibold transition-all">
          ...
          {!loading && !error && board.map((entry) => {
            ...
            return (
              <div key={player?._id || Math.random()} className={`flex items-center justify-between p-4 rounded-2xl border ${isMe ? 'bg-blue-900/20 border-blue-500/50' : 'bg-gray-800/50 border-gray-700/50'}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: player?.color || '#6b7280' }} />
                      <p className={`font-bold ${isMe ? 'text-white' : 'text-gray-200'}`}>
                        {player?.name || 'Unknown Player'} {isMe && ' (You)'}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><Map size={12} /> {Math.round(entry.areaConquered || 0)}m²</span>
                      <span className="flex items-center gap-1"><Activity size={12} /> {Math.round(entry.totalDistance || 0)}m</span>
                    </div>
                  </div>
                </div>
        ...
        {!loading && !error && myRank && (
          <div className="absolute bottom-0 left-0 w-full bg-gray-800 border-t border-gray-700 p-4 shadow-[0_-10px_20px_rgba(0,0,0,0.3)]">
            ...
            <div><p className="text-2xl font-black text-white">Rank #{myRank}</p></div>
          </div>
        )}
      </div>
    </>
